...
public class OpenClawUtils {
    ...
    private static final String PROJECT_TAG = "QChatApp";
     ...
     *
     * @param context 上下文对象
     * @param cloud   是否使用云端会话（true=云端，false=本地）
     */
    public static void checkAndCreateOpenClawSession(Context context, boolean cloud) {
        try {
            ...
            if (TextUtils.isEmpty(openClawAccount)) {
                ALog.i(PROJECT_TAG, "checkAndCreateOpenClawSession", "No OpenClaw account configured, skipping");
                return;
            }

            String conversationId = ConversationIdUtils.conversationId(
                    openClawAccount,
            ...
            if (cloud) {
                ConversationRepo.getConversation(
                        ...
                        new FetchCallback<V2NIMConversation>() {
                            ...
                            public void onError(int i, @Nullable String s) {
                                ALog.e(PROJECT_TAG, "checkAndCreateOpenClawSession",
                                        "Failed to get conversation: " + s);
                                // 创建欢迎消息
                ...
                LocalConversationRepo.getConversation(
                        ...
                        new FetchCallback<V2NIMLocalConversation>() {
                            ...
                            public void onError(int i, @Nullable String s) {
                                ALog.e(PROJECT_TAG, "checkAndCreateOpenClawSession",
                                        "Failed to get local conversation: " + s);
                                // 创建欢迎消息
                                OpenClawUtils.createOpenClawWelcomeMessage(context, openClawAccount);
                            }

                            @Override
                            public void onSuccess(@Nullable V2NIMLocalConversation v2NIMLocalConversation) {
                                // 会话不存在时创建欢迎消息
            ...
            // 静默处理异常，不影响主流程
            ALog.e(PROJECT_TAG, "checkAndCreateOpenClawSession", "Error occurred");
        }
    }
    ...
    public static void createOpenClawWelcomeMessage(Context context, String openClawAccount) {
        ...
        try {
            ...
            if (messageService != null) {
                messageService.insertMessageToLocal(
                        ...
                        new V2NIMFailureCallback() {
                            ...
                            public void onFailure(V2NIMError error) {
                                ALog.e(TAG, "createOpenClawWelcomeMessage insertMessageToLocal",
                                        error.getDesc());
                            }
                        });
                ALog.i(TAG, "createOpenClawWelcomeMessage",
                ...
                ALog.i(TAG, "createOpenClawWelcomeMessage", "MessageService not available");
            }
        } catch (Exception e) {
            ALog.e(TAG, "createOpenClawWelcomeMessage", "Failed to create welcome message");
        }
    }
